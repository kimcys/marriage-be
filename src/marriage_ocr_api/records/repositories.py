...
def _get_record_by_key(session: Session, *, job_id: UUID, source_key: str) -> OCRRecord | None:
...
def create_record(
    ...
    *,
    ...
    existing = _get_record_by_key(session, job_id=job_id, source_key=source_key)
...
def create_record_if_missing(
    ...
    *,
    ...
    if _get_record_by_key(session, job_id=job_id, source_key=source_key) is not None:
        return False
